...
def analysis(request):
    ...
    for item_var_a in cursor_var_a:
        list_var_a.append(item_var_a)      

    item_var_a_get = list(map(itemgetter(0), list_var_a))
    item_var_a_get_asc = sorted(item_var_a_get)
    print(item_var_a_get_asc)
    
    uniq = []
    for str in item_var_a_get_asc:
        if str not in uniq:
            uniq.append(str)
    print(uniq)
